...
from pregated_moe_common import (
    apply_pregated_runtime_defaults,
    build_pregated_moe_runtime,
    cleanup_pregated_moe_runtime,
    resolve_image_root,
    run_moe_sample,
    run_moe_warmup,
    save_runtime_trace,
...
def _run_dataset_benchmark(
    ...
    print("")
    print("=" * 80)
    print(f"[{dataset_index}/{total_datasets}] {MODEL_SPECS[args.model]['label']} Pregated-MoE | Benchmark: {benchmark}")
    print("=" * 80)
    ...
    for idx, row in tqdm(
        dataset.iterrows(),
        total=len(dataset),
        desc=f"{args.model}-pregated_moe-{benchmark}",
    ):
        sample_id = _resolve_sample_id(row, idx)
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = _default_output_dir("pregated_moe", args.model)
    ...
    print("\n" + "=" * 80)
    print(f"{MODEL_SPECS[args.model]['label']} Pregated-MoE test")
    print("=" * 80)
    ...
    runtime = None
    try:
        runtime = build_pregated_moe_runtime(args)
        ...
        cleanup_pregated_moe_runtime(runtime)
