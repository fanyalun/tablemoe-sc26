#!/usr/bin/env python3
"""
Verify that the Pregated-MoE prefetch extension was built correctly
...
print("=" * 60)
print("Pregated-MoE Build Verification")
print("=" * 60)

# 1. Check that the .so file exists
print("\n1. Shared library exists:")
so_files = []
for root, _, files in os.walk("pregated_moe"):
...
try:
    ...
    if so_path:
        ...
        if spec and spec.loader:
            ...
            sys.modules["pregated_moe._store"] = _store
            spec.loader.exec_module(_store)
            check("import pregated_moe._store", True)
        else:
            check(
                "import pregated_moe._store",
                False,
    ...
    check("import pregated_moe._store", False, str(e))
