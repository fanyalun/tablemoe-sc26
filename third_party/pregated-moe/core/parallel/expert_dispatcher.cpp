...
void ExpertDispatcher::Enqueue(CallArgs& args) {
  ...
  if (expert_node->node == nullptr || !expert_node->node->device.is_cuda()) {
    auto node_desc = (expert_node->node == nullptr) ? std::string("nullptr")
                                                    : expert_node->node->str();
    if (expert_node->node != nullptr) {
      DLOG_ERROR(
          "ExpertDispatcher::Enqueue debug expert_idx ", expert_idx,
          " layer_idx ", layer_idx, " pin ",
          expert_node->node->pin_count.load(), " active ",
          expert_node->node->active_users.load(), " last_access ",
          expert_node->node->last_access_time);
    }
    DLOG_FATAL(
        "ExpertDispatcher::Enqueue: expert is not resident on GPU "
        "(expert_idx ",
  ...
  expert_node->node->active_users.fetch_add(1);
...
void ExpertDispatcher::NotifyFetchStart() {
  // Exact prepare path no longer uses the fetch queue.
}
...
void ExpertDispatcher::OutputFunc(ExecArgs args, torch::Tensor output,
  ...
  args.expert_node->node->active_users.fetch_sub(1);
