...
from pregated_moe.models.modeling_arctic import ArcticConfig
...
class MoE:
    ...
    Args:
        ...
        config (`Dict` or `os.PathLike`): The Pregated-MoE configuration. It can be:
    ...
    def __init__(
        ...
        if "arctic" in model_name_or_path:
            ...
            for name in ("deepseek-vl2", "deepseek_vl2", "deepseekvl2")
        ):
            ensure_local_deepseek_vl2_repo()
        ...
        try:
            ...
            print(
                "[WARNING] FlashAttention is not available in the current environment. Falling back to SDPA."
            )
        with self.engine.init(cls=model_cls, ar_config=config):
            if arch == "deepseek_vl2":
    ...
    def _configure_hook(self, input_ids: torch.LongTensor):
        ...
        if self.arch == "grok":
            pregated_moe.models.modeling_grok.modeling_grok1.apply_rotary_pos_emb = apply_rotary_pos_emb

        if self.arch == "arctic":
            pregated_moe.models.modeling_arctic.modeling_arctic.apply_rotary_pos_emb = apply_rotary_pos_emb

        if self.arch == "deepseek" or self.arch == "deepseek_v3":
            pregated_moe.models.modeling_deepseek_v2.modeling_deepseek.apply_rotary_pos_emb = apply_rotary_pos_emb_deepseek
            pregated_moe.models.modeling_deepseek_v3.modeling_deepseek.apply_rotary_pos_emb = apply_rotary_pos_emb_deepseek
            # apply_rotary_pos_emb is defined in deepseek and differs from this version.
        if self.arch == "deepseek_vl2":
            ensure_local_deepseek_vl2_repo()
            import deepseek_vl2.models.modeling_deepseek as deepseek_vl2_modeling

            deepseek_vl2_modeling.apply_rotary_pos_emb = (
                apply_rotary_pos_emb_deepseek
            )
