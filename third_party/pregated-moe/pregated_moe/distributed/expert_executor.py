...
from pregated_moe.utils import ArcherConfig
...
class DistributedExpertExecutor:
    ...
    def dispatch_local(
        ...
        if router_mask.device != hidden_states.device:
            raise RuntimeError(
                "router_mask device mismatch: "
                f"hidden_states={hidden_states.device}, "
                f"router_mask={router_mask.device}"
            )
        if router_weights.device != hidden_states.device:
            raise RuntimeError(
                "router_weights device mismatch: "
                f"hidden_states={hidden_states.device}, "
                f"router_weights={router_weights.device}"
            )
        ...
        if self.debug_enabled:
            print(
                f"[PregatedExecDebug] layer={layer_id} experts={expert_list[:32]} total={len(expert_list)}",
                flush=True,
            )
