...
class DeepseekVLV2ForCausalLM(DeepseekVLV2PreTrainedModel):
    ...
    def __init__(self, config: DeepseekVLV2Config):
        ...
        language_config._attn_implementation = config._attn_implementation  # 将attn_implementation传递给语言模型配置，以支持Flash Attention 2
    ...
    def prepare_inputs_embeds(
        ...
        h = w = int(hw ** 0.5)
        direct_param_device = images_embeds.device
        direct_param_dtype = images_embeds.dtype
        image_newline = getattr(self, "image_newline", None)
        if image_newline is not None:
            image_newline = image_newline.to(
                device=direct_param_device,
                dtype=direct_param_dtype,
            )
        view_seperator = getattr(self, "view_seperator", None)
        if view_seperator is not None:
            view_seperator = view_seperator.to(
                device=direct_param_device,
                dtype=direct_param_dtype,
            )
        tile_indicators = getattr(self, "tile_indicators", None)
        if tile_indicators is not None:
            tile_indicators = tile_indicators.to(
                device=direct_param_device,
                dtype=direct_param_dtype,
            )
        ...
        if images_seq_mask is not None:
            images_seq_mask = images_seq_mask.to(device=input_embeds.device)
            ...
            for jdx in range(images_spatial_crop.shape[1]):
                ...
                if self.tile_tag == "2D":
                    ...
                    global_features = torch.cat(
                        [tile_indicators[0:1], global_features], dim=0
                    )
                    local_features = torch.cat(
                        [tile_indicators[1:num_tiles_in_image + 1].unsqueeze(1), local_features], dim=1
                    )
