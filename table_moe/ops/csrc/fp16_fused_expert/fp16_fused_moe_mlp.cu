...
// sm_86 budget: (64×64 + 64×64) FP16 × 3 stages = 48 KB/TB → 2 TBs/SM.
...
// NOTE: CUTLASS 2.x DefaultGemm only specialises on Sm80 for FP16 tensor ops;
...
void fused_moe_ffn_fp16_into(torch::Tensor& hidden,     // [M, H]
                             ...
                             cudaStream_t stream) {
  TORCH_CHECK(hidden.scalar_type() == at::kHalf,
              "fused_moe_ffn_fp16_into: FP16 only");
  ...
  TORCH_CHECK(H == H_out, "fused_moe_ffn_fp16_into: hidden dim mismatch");
  TORCH_CHECK(gate_proj.size(1) == H && up_proj.size(1) == H,
              "fused_moe_ffn_fp16_into: gate/up proj K-dim mismatch");
  TORCH_CHECK(down_proj.size(1) == I,
              "fused_moe_ffn_fp16_into: down proj intermediate dim mismatch");
  ...
  if (use_large_k) {
    ...
    // ------------------------------------------------------------------
    {
      ...
      TORCH_CHECK(status == cutlass::Status::kSuccess,
                  "fused_moe_ffn_fp16_into GEMM0-LK (gate) failed: ",
    ...
    // ------------------------------------------------------------------
    {
      ...
      cutlass::Status status = gemm(args, nullptr, stream);
      TORCH_CHECK(status == cutlass::Status::kSuccess,
                  "fused_moe_ffn_fp16_into GEMM1-LK (up+silu-mul) failed: ",
                  cutlassGetStatusString(status));
    }
    ...
    // ------------------------------------------------------------------
    {
      ...
      cutlass::Status status = gemm(args, nullptr, stream);
      TORCH_CHECK(status == cutlass::Status::kSuccess,
                  "fused_moe_ffn_fp16_into GEMM2-LK (down) failed: ",
                  cutlassGetStatusString(status));
    }
  } else {
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    {
      ...
      cutlass::Status status = gemm(args, nullptr, stream);
      TORCH_CHECK(status == cutlass::Status::kSuccess,
                  "fused_moe_ffn_fp16_into GEMM0 (gate) failed: ",
                  cutlassGetStatusString(status));
    }
    ...
    // ------------------------------------------------------------------
    {
      ...
      cutlass::Status status = gemm(args, nullptr, stream);
      TORCH_CHECK(status == cutlass::Status::kSuccess,
                  "fused_moe_ffn_fp16_into GEMM1 (up+silu-mul) failed: ",
                  cutlassGetStatusString(status));
    }
    ...
    // ------------------------------------------------------------------
    {
      ...
      cutlass::Status status = gemm(args, nullptr, stream);
      TORCH_CHECK(status == cutlass::Status::kSuccess,
                  "fused_moe_ffn_fp16_into GEMM2 (down) failed: ",
                  cutlassGetStatusString(status));
    }
  }
}
